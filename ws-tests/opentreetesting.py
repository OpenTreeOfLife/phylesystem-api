...
def config(section=None, param=None, default=None):
    ...
    try:
        ...
        if default != None:
            ...
            sys.stderr.write(
                'Config file "%s" does not contain option "%s" in section "%s"\n'
    ...
    for arg in sys.argv[1:]:
        ...
        if len(equatands) == 2:
            ...
            if len(sec_param) == 2:
                ...
                sys.stderr.write(
                    "Command line argument %s not in form section:parameter=value"
                    % (arg)
                )
        else:
            sys.stderr.write(
                "Command line argument %s not in form section:parameter=value" % (arg)
            )
...
def get_obj_from_http(url, verb="GET", data=None, params=None, headers=None):
    ...
    if data:
        ...
        resp = requests.request(
            verb, translate(url), headers=headers, params=params, allow_redirects=True
        )
...
def test_http_json_method(
    ...
    if data:
        if verb == "GET":
            resp = requests.get(
                translate(url), headers=headers, params=data, allow_redirects=True
            )
        else:
            resp = requests.request(
                verb,
    ...
    if (not exps) and (
        ...
        or isinstance(expected_status, tuple)
        or isinstance(expected_status, set)
    ):
    ...
    if not exps:
        ...
        debug(
            "Got status code {c} (expecting {e})".format(
                c=resp.status_code, e=expected_status
            )
        )
        debug(
            "Did not get expected response status. Got:\n{s}".format(s=resp.status_code)
        )
        debug("Full response: {r}".format(r=resp.text))
        raise_for_status(resp)
    ...
    if expected_response is not None:
        ...
        try:
            ...
            if results != expected_response:
                debug(
                    "Did not get expect response content. Got:\n{s}".format(s=resp.text)
                )
                return fail_return
        except:
...
def raise_for_status(resp):
    try:
        ...
        try:
            j = resp.json()
            m = "\n    ".join(
                ['"{k}": {v}'.format(k=k, v=v) for k, v in list(r.items())]
            )
...
def writable_api_host_and_oauth_or_exit(fn):
    ...
    if apihost.startswith("https://api.opentree"):
        debug(
            "{} skipped because host is set to production api server. This test must be run locally or against devapi.\n".format(
                fn
            )
        )
        # This coordinates with run_tests.sh
